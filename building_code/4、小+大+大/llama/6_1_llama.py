...
def main():
    ...
    for idx, row in roberta_df.iterrows():
        ...
        if skip_num == "未知":
            print(f"[WARN] small-model predict={predicted_label1}, 无法映射 => skip")
            continue
        if test_text not in fusion_map:
            print(f"[WARN] text not in fusion_map => {test_text[:20]}... skip")
            continue
        cat_dict = fusion_map[test_text]

        candidate_samples_str = ""
        print(f"\n=== 第{idx+1}/{len(roberta_df)}条文本 ===")
        print(f"文本[:25]: {test_text[:25]}")
        ...
        base.append({"role":"user","content":real_prompt})

        run_labels = []
        run_confusions = []
        for run_id in range(3):
            reply = call_llama(copy.deepcopy(base), max_new_tokens=512)
            print(f"[LLAMA 第{run_id+1}次] reply =>\n", reply, "\n")
            m_label = re.search(r"备选类别为\s*[:：]\s*([^\n]+)", reply)
            alt_label = m_label.group(1).strip() if m_label else "未知"
            m_conf = re.search(r"混淆点为\s*[:：]\s*([\s\S]+)", reply)
            confusion_point = m_conf.group(1).strip() if m_conf else ""
            run_labels.append(alt_label)
            run_confusions.append(confusion_point)

        voted_alt = majority_vote(run_labels)
        chosen_confusion = ""
        for l, c in zip(run_labels, run_confusions):
            if l == voted_alt:
                chosen_confusion = c
                break

        final_result = "正确" if true_label in [predicted_label1, voted_alt] else "错误"
        results_list.append({
            "text": test_text,
            "predicted_label1": predicted_label1,
            "predicted_label2": voted_alt,
