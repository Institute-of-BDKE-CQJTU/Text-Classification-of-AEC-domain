...
def evaluate_and_save_results(model, data_loader, device, output_file, label_list):
    model.eval()
    results = []
    with torch.no_grad():
        for batch in tqdm(data_loader, desc="Evaluating"):
            ...
            for i in range(len(true_labels)):
                ...
                predicted_label_name = label_list[pred_labels[i]]
                result = {
                    "text": texts[i],
                    "true_label": true_label_name,
                    "predicted_label": predicted_label_name,
                    "result": "正确" if true_label_name == predicted_label_name else "错误"
                }
...
def main():
    ...
    evaluate_and_save_results(model, test_loader, device, output_file, label_list)
