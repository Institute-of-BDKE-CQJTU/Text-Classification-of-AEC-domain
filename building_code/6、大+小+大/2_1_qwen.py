...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        for run_id in range(3):
            ...
            final_category_once = match_final.group(1).strip() if match_final else "未知"
            match_reason = re.search(r"理由是\s*[:：]\s*([^\n]+)", reply)
            reason_once = match_reason.group(1).strip() if match_reason else "未匹配到"
            runs.append((final_category_once, reason_once))

        vote = {}
        for cat, _ in runs:
            vote[cat.lower()] = vote.get(cat.lower(), 0) + 1
        ...
        final_reason = random.choice(reasons) if reasons else "未匹配到"
